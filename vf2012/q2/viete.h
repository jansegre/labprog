...
//
// v(0) = sqrt(2) / 2
// v(1) = (sqrt(2) / 2) * (sqrt(2 + sqrt(2)) / 2)
//      = v(0) * sqrt(2 + sqrt(2)) / 2
// v(2) = v(1) * sqrt(2 + sqrt(2 + sqrt(2))) / 2
//
// Note que precisamos generalizar mais um termo, a parte
// que está dentro da raiz, note como esse termo se comporta:
//
// r(0) = sqrt(2)
// r(1) = sqrt(2 + sqrt(2)) = sqrt(2 + r(0))
// r(2) = sqrt(2 + sqrt(2 + sqrt(2))) = sqrt(2 + r(1))
//
...
// termos iniciais
template<> const double viete<0>::r = std::sqrt(2);
template<> const double viete<0>::v = std::sqrt(2) / 2;
